...
// const safetyScore = 72; // example score (0–100)
...
const Safety = () => {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/user/reports`);
        ...
        reports.forEach((report) => {
          ...
          <div style="font-size:13px">
            <p><strong>Incident:</strong> ${mapIncident(
              report.whatHappened
            )}</p>
    ...
    const fetchScore = async (latitude, longitude) => {
      try {
        ...
        const res = await fetch(
          `${import.meta.env.VITE_BACKEND_URL}/user/safeScore?lat=${latitude}&lng=${longitude}`,
          {
            headers: {
